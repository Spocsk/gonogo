...
export async function POST(request: Request, ctx: Ctx) {
  ...
  let body: { playerId?: string; choiceIndex?: number }
  try {
    body = await request.json()
  } catch {
    return NextResponse.json({ error: "JSON invalide." }, { status: 400 })
  }
  if (!body.playerId || typeof body.choiceIndex !== "number") {
    return NextResponse.json({ error: "playerId et choiceIndex requis." }, { status: 400 })
  }

  try {
    const result = await withGameLock(pin, async (game) => {
      if (!game) return { status: 404 as const, error: "Lien sol introuvable." }
      const next = answerQuestion(game, body.playerId!, body.choiceIndex!)
      await saveGame(next)
      return {
        status: 200 as const,
